...
class CustomHomeBody extends StatelessWidget {
 ...
 /* BlocBuilder<TaskCubit, TaskState>(
                builder: (context, state) {
                  ...
                  } else if (state is GetTasksListSuccessState ||
                      BlocProvider.of<TaskCubit>(context).allTasks.isNotEmpty) {
                    final indexSelected =
                        BlocProvider.of<TaskCubit>(context).indexNumSelected;
                    var tasksList = <CreateTaskEntity>[];
                    switch (indexSelected) {
                      case 0:
                        tasksList =
                            BlocProvider.of<TaskCubit>(context).allTasks;
                        break;
                      case 1:
                        tasksList =
                            BlocProvider.of<TaskCubit>(context).inPogress;
                        break;
                      case 2:
                        tasksList = BlocProvider.of<TaskCubit>(context).waiting;
                        break;
                      case 3:
                        tasksList =
                            BlocProvider.of<TaskCubit>(context).finished;
                        break;
                      default:
                    }
                    return CustomTaskListView(
                      tasks: tasksList,
                    );
                  } else if (state is GetTasksListErrorState) {
                    return Expanded(
                  ...
                  } else if (BlocProvider.of<TaskCubit>(context)
                      .allTasks
                      .isEmpty) {
                    return Expanded(
                      child: CustomEmptyListWidget(
